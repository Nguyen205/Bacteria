...
def write_sizes(height : float, width : float):
    """
    Updates the essentially-global height and width for this program
    """
...
def write_cutoff(cutoff : bool, side_cutoff : bool):
    """
    Updates the essentially-global cutoffs for this program
    """
...
def write_bw(data : list[ImageData]):
    """
    Finds all color thresholds based on a magic cutoff
    Effectively converts the image to black-and-white, 
      where sufficiently "gray" colors are sent to one extreme or the other
    Updates each image.bw in data with the resulting image
    """
    # magic numbers, hooray!  Used for offset, based on ImageJ survey apparently
    COLOR_CUTOFF = 2 / 256
...
def read_bw(data : list[ImageData]):
    """
    Reads a provided BW file into ImageData
    May be called in place of write_bw
    """
...
def write_bwc(data : list[ImageData]):
    """
    Calculates cutoff in image_mod based on current image data
    Updates each image.bwc in data with the resulting image
    Writes each resulting image
    """
...
def read_bwc(data : list[ImageData]):
    """
    Reads a provided BWC file into ImageData
    May be called in place of write_bwc
    """
...
def calculate_outliers(data : list[ImageData]):
    """
    Calculates all outliers associated with the current data.bwc
    Removes outliers and writes the resulting information to 
      data.outl, data.outl2, and data.outl3
    """
    print_normal('Determining outliers')
    # TODO: _height fix, it's a bit magic right now
    #  Seems to work, but could be cleaned up
    out_area = 360
...
def oct_analysis(data : list[ImageData]):
    """
    Calculates statistical information about finalized images
    Writes this result to the options-specified CSV file
    """
