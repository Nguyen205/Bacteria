sens_an_spearman = function(parcel, iter){
  ...
  # drawn from multivariate to preserve covariance
  # V diam in um
  # * Deposition efficiency (unitless, uniform) * Fraction to size i (unitless, point)
  
  C = mvlogn(iter, 
             mu = array(parcel[['Caer']]$aer_mean), 
             sd = array(parcel[['Caer']]$aer_sd), 
             corrMatrix = parcel[['aer_corr']])
  ...
  CVDF = list()
  for (i in 1:10){ # 10 aerosol bin sizes
    CVDF[[i]] = C$samples[,i]*V[i,1]*Dep_eff[[i]]*Frac_Leg[i]
  }
  ...
  ind_var = c(list(C_0), list(C_rat), list(B), list(t), list(f), list(tf), 
              list(C$samples[,1]), list(C$samples[,2]), list(C$samples[,3]),
              list(C$samples[,4]), list(C$samples[,5]), list(C$samples[,6]),
              list(C$samples[,7]), list(C$samples[,8]), list(C$samples[,9]),
              list(C$samples[,10]), list(Dep_eff[[1]]), list(Dep_eff[[2]]), 
