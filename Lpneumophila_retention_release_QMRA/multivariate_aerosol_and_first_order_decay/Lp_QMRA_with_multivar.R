...
library(EnvStats)
library(truncnorm)
#devtools::install_github("AlessandroDeCarlo27/mvLognCorrEst")
library(mvLognCorrEst)

#-------------------------------------RUN MONTE CARLO--------------------------
source('prep_parc_with_multivar.R')
source('run_mc_with_multivar.R')
...
activity_opt = c('drink','handwash', 'dishwash')
...
write.table(fix_inf, file = 'results/with_multivar_fixture_inf.csv', row.names = FALSE, sep = ',')
write.table(dai_inf, file = 'results/with_multivar_daily_inf.csv', row.names = FALSE, sep = ',')
write.table(ann_inf, file = 'results/with_multivar_annual_inf.csv', row.names = FALSE, sep = ',')
write.table(fix_ill, file = 'results/with_multivar_fixture_ill.csv', row.names = FALSE, sep = ',')
write.table(dai_ill, file = 'results/with_multivar_daily_ill.csv', row.names = FALSE, sep = ',')
write.table(ann_ill, file = 'results/with_multivar_annual_ill.csv', row.names = FALSE, sep = ',')
...
# Load data
fix_inf = read.csv('results/with_multivar_fixture_inf.csv', header = TRUE)
dai_inf = read.csv('results/with_multivar_daily_inf.csv', header = TRUE)
ann_inf = read.csv('results/with_multivar_annual_inf.csv', header = TRUE)
fix_ill = read.csv('results/with_multivar_fixture_ill.csv', header = TRUE)
dai_ill = read.csv('results/with_multivar_daily_ill.csv', header = TRUE)
ann_ill = read.csv('results/with_multivar_annual_ill.csv', header = TRUE)
...
for(r in 1:length(risk_type)){
  for(a in 1:length(activity_opt)){
    ...
    f = paste('results/with_multivar', 'comp1_', activity_opt[a], '_', risk_type[r], '.csv', sep = '')
...
#-------------------------------------CALC STATS--------------------------------
source('calc_stats_with_multivar.R')
source('round_robin_wilk_with_multivar.R')
...
for(i in 1:length(inf_ill_opt)){
  ...
  f = paste('results/with_multivar_stats_',inf_ill_opt[i], '.csv', sep = '')
  ...
  for(j in 1:length(targ_var_opts)){
    ...
    f2 = paste('results/with_multivar_stats_wilk_',inf_ill_opt[i], '_', 
...
source('sens_an_spearman_with_multivar.R')
filter_age_opt = c('none_Clwithout', 'none_Clwith', 'new_GW', 'new_PO', 'old_GW', 'old_PO')
age_opt = c('child', 'adult', 'elderly')
activity_opt = c('drink','handwash', 'dishwash')
...
write.table(spearman_btwn, file = 'results/with_multivar_spearman_btwn_fixture.csv', row.names = TRUE, sep = ',')
