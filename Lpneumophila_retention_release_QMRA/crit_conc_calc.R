crit_conc_calc = function(parcel, Lp_range, risk_type){
  days = 365
...
#---------------------------iterate through Lp ranges----------------------------
  
  for(r in 1:length(Lp_range)){
    
#------------------------------Generate variables-------------------------------
    # L pneumophila concentration (copies/mL)
    C_0 = Lp_range[r]
    ...
    # Concentration of aerosols (#/mL, lognormal) * V aerosols (mL/#)
    C = list()
...
#-------------------------------------Dose---------------------------------------
    ...
    # Calculate fixture dose = C_Leg*B*t*sum(C_aer*V_aer)*sum(Dep_eff*Frac_Leg)
    Fixture_dose = C_times_t*B*sum_CVDF
    
    # Calculate daily dose = C_Leg*B*t*f*sum(C_aer*V_aer)*sum(Dep_eff*Frac_Leg)
...
#--------------------Append risks to variable for each iteration----------------
  ...
  if(risk_type == 'fixture'){
    output_df = data.frame(fixture_ill)
  } else if(risk_type == 'daily'){
    output_df = data.frame(daily_ill)
  } else if(risk_type == 'annual'){
    output_df = data.frame(annual_ill)
  } else {
    stop('Error: invalid risk_type')
  }
